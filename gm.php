    ...
    class event
    {
        ...
        public static function _systeminfo_api()
        {
            $_display = [
                ...
                'task' => \zslm\system\system_info::task_list(),
            ];
        ...
        public static function _vps_start()
        {
            \zslm\vps\api::init(\sys\config::get('vps_url'), \sys\config::get('vps_key'), \sys\config::get('vps_hash'), \sys\config::get('vps_type'));
            \zslm\vps\api::start();
            \sys\send::send(data::msg(1, 'ok'));
        }
        public static function _vps_stop()
        {
            \zslm\vps\api::init(\sys\config::get('vps_url'), \sys\config::get('vps_key'), \sys\config::get('vps_hash'), \sys\config::get('vps_type'));
            \zslm\vps\api::stop();
            \sys\send::send(data::msg(1, 'ok'));
        }
        public static function _vps_restart()
        {
            \zslm\vps\api::init(\sys\config::get('vps_url'), \sys\config::get('vps_key'), \sys\config::get('vps_hash'), \sys\config::get('vps_type'));
            \zslm\vps\api::restart();
            \sys\send::send(data::msg(1, 'ok'));
        }
        public static function _vps_kill()
        {
            \zslm\vps\api::init(\sys\config::get('vps_url'), \sys\config::get('vps_key'), \sys\config::get('vps_hash'), \sys\config::get('vps_type'));
            \zslm\vps\api::kill();
            \sys\send::send(data::msg(1, 'ok'));
        }
        public static function _systeminfo_restart()
        {
            \sys\send::send(data::msg(0, '在fpm模式下无此命令特权,暂不可用'));
...
namespace sys {
    ...
    {
        return \zslm\get_disk_total($filesize);
    }
                                ...
                                <table class="layui-table">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>操作</td>
                                            <td>
                                                <blockquote class="layui-elem-quote">谨慎点击</blockquote>
                                                <button class="layui-btn layui-btn-normal layui-btn-sm vps_click">start</button>
                                                <button class="layui-btn layui-btn-normal layui-btn-sm vps_click">stop</button>
                                                <button class="layui-btn layui-btn-normal layui-btn-sm vps_click">restart</button>
                                                <button class="layui-btn layui-btn-normal layui-btn-sm vps_click">kill</button>
                                                
                                                <script>
                                                    $('.vps_click').click(function(){
                                                        $.get("?click=vps_" + $(this).html(),
                                                        function (result) {
                                ...
                                <div class="layui-tab layui-tab-card">
                                    ...
                                    <div class="layui-tab-content">
                                            ...
                                            <table class="layui-table">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        <td>操作</td>
                                                        <td>
                                                            <button class="layui-btn layui-btn-normal layui-btn-sm" onclick="layer.msg('出于操作方便性考虑暂不开启此功能');">stop</button>
                                                            <button class="layui-btn layui-btn-normal layui-btn-sm vps_click">restart</button>
                                                            <script>
                                                                $('.vps_click').click(function(){
                                                                    $.get("?click=systeminfo_" + $(this).html(),
                                                                    function (result) {
                                                                        layer.msg(result.msg);
                                                                    }, "json")
                                                                    .fail(function () {
                                                                        layer.msg('请求错误,但是原因也可能是服务器执行命令后无法响应消息了');
                                                                    });
                                                                });
                                        ...
                                        <div class="layui-tab-item">
                                            ...
                                            <table class="layui-table">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="status_network"><i class="layui-icon layui-icon-up layui-icon-up-color"></i> 0kb/s 丨 <i class="layui-icon layui-icon-down layui-icon-down-color"></i> 0kb/s</td>
                                                    </tr>
                                                    <tr>
                                                        <td>实时进程网速</td>
                                                        <td>
                                            ...
                                            <blockquote class="layui-elem-quote">进程信息</blockquote>
                                            <table class="layui-table">
                                                <colgroup>
                                                    <col>
                                                    <col>
                            ...
                            $.get("?click=systeminfo_api",
                                function (result) {
                                    if (result.code == '0') {
                                        ...
                                        $('.status_network').html('<i class="layui-icon layui-icon-up layui-icon-up-color"></i> ' + result.msg.net.send + '/s 丨 <i class="layui-icon layui-icon-down layui-icon-down-color"></i> ' + result.msg.net.recv + '/s');
                                        var display_html = "";
                                        for (var i = 0 in result.msg.task) {
                                            display_html = display_html + '\
                                            <tr>\
                                                <td>' + result.msg.task[i].name + '</td>\
                                                <td>' + result.msg.task[i].pid + '</td>\
                                                <td>' + result.msg.task[i].cpunum + '</td>\
                                                <td>' + result.msg.task[i].cpu + '</td>\
                                                <td>' + result.msg.task[i].mem + '</td>\
                                                <td>' + result.msg.task[i].cmd + '</td>\
                                            </tr>';
                                        }
                                        $('.task_list').html(display_html);
                                        get_sys_status();
                                    }else{
                                        status_run == false;
...
namespace zslm\system {
    class system_info
    {
        ...
        public static function network()
        {
            ...
            return ['recv' => $net_2[0] - $net_1[0], 'send' => $net_2[8] - $net_1[8]];
        }
        ...
        public static function cpu_net_api()
        {
            ...
            return [
                ...
                'net' => ['recv' => \zslm\get_disk_total($net_2[0] - $net_1[0]), 'send' => \zslm\get_disk_total($net_2[8] - $net_1[8])]
            ];
        }
...
namespace zslm\vps {
    ...
    class api
    {
        ...
        public static function init($url, $key, $hash, $type = 0)
        {
            self::$type = $type;
            if ($type == 0) {
                self::$url = $url;
                self::$key = sprintf('?veid=%s&api_key=%s', $key, $hash);
            } else {
                self::$url = self::$url = sprintf('%s?key=%s&hash=%s&action=', $url, $key, $hash);
            }
        }
        private static function get_contents_solusvm($call, $time_out = 10)
        {
            $data = get_contents(self::$url . $call, time_out: $time_out);
        ...
        private static function get_contents_kiwivm($call, $time_out = 10)
        {
            $data = get_contents(self::$url . $call . self::$key, time_out: $time_out);
            $ret  = json_decode($data, true);
            unset($ret['available_isos']);
            return $ret;
        }
        private static function get_contents($call, $time_out = 10)
        {
            if ($call == null) return false;
            return self::$type == 0 ? self::get_contents_kiwivm($call, $time_out) : self::get_contents_solusvm($call, $time_out);
        }
        public static function info()
        {
            $info = self::get_contents((self::$type == 0 ? 'getServiceInfo' : 'info&ipaddr=true&bw=true&mem=true&hdd=true'), 3);
            if (self::$type == 0) {
                $ret  = [];
                $ret['api_type'] = 'kiwivm'; # kiwivm solusvm
                $ret['type']     = $info['vm_type'] ?? '获取失败';
                $ret['ip']       = $info['ip_addresses'][0] ?? '获取失败';
                $ret['os']       = $info['os'] ?? '获取失败';
                $ret['hostname'] = $info['hostname'] ?? '获取失败';
                $ret['node_ip']  = $info['node_ip'] ?? '获取失败'; // ip地址的物理节点
                $ret['location'] = $info['node_datacenter'] ?? '获取失败';
                $ret['disk_max'] = $info['plan_disk'] ?? 1; // 磁盘大小
                $ret['ram_max']  = $info['plan_ram'] ?? 1;  // 内存大小
                $ret['net_max']  = $info['plan_monthly_data'] ?? 1; // 总流量
                $ret['net_use']  = $info['data_counter'] ?? 1; // 已使用

                $info = self::status();
                $ret['disk_use'] = $info['ve_used_disk_space_b'] ?? 1;
                $ret['ram_use']  = $info['mem_available_kb'] ?? 1 * 1024;
                ...
                $ret['status']   = $info['status'] ?? '获取失败';
                return $ret;
            }
        }
        private static function status()
        {
            return self::get_contents((self::$type == 0 ? 'getLiveServiceInfo' : 'status'), 3);
        }
        public static function start()
        {
            self::get_contents((self::$type == 0 ? 'start' : 'boot'), 3);
            return true;
        }
        public static function stop()
        {
            self::get_contents((self::$type == 0 ? 'stop' : 'shutdown'), 3);
            return true;
        }
        public static function restart()
        {
            self::get_contents((self::$type == 0 ? 'restart' : 'reboot'), 3);
            return true;
        }
        public static function kill()
        {
            self::get_contents((self::$type == 0 ? 'kill' : null), 3);
            return true;
        }
    }
}
